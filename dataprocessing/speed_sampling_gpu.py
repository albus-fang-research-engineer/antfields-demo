...
def speed2d(X, v_obs, f_obs, offset, margin):

    #v_obs = torch.tensor(v_obs, dtype=torch.float32, device='cuda')
    #f_obs = torch.tensor(f_obs, dtype=torch.long, device='cuda')
    #t_obs = v_obs[f_obs].unsqueeze(dim=0)

    v_ind = igl.boundary_loop(f_obs)
    Points = v_obs[v_ind,:3]
    #print(Points)
    id = np.arange(Points.shape[0])

    LineSegments = np.concatenate((np.expand_dims(id[:-1],axis=1),
                                   np.expand_dims(id[1:],axis=1),
                                   np.expand_dims(id[1:],axis=1)),axis=1)

    v_obs = torch.tensor(Points, dtype=torch.float32, device='cuda')
    f_obs = torch.tensor(LineSegments, dtype=torch.long, device='cuda')
    t_obs = v_obs[f_obs].unsqueeze(dim=0)
    
    obs_distance = point_obstacle_distance(X, t_obs) - offset
    #N_list = []


    #sampled_points = X.detach().cpu().numpy()
    distance = obs_distance.detach().cpu().numpy()
    #normal = N.detach().cpu().numpy()
    
    speed = np.clip(distance/margin , a_min = offset/margin, a_max = 1)
    
    return speed

def speed3d(X, v_obs, offset, margin):

    v_obs = torch.tensor(v_obs, dtype=torch.float32, device='cuda')
    #f_obs = torch.tensor(f_obs, dtype=torch.long, device='cuda')
    f_obs = torch.zeros((v_obs.shape[0],3), dtype=torch.long, device='cuda')
    f_obs[:,0] = torch.arange(v_obs.shape[0])
    f_obs[:,1] = torch.arange(v_obs.shape[0])
    f_obs[:,2] = torch.arange(v_obs.shape[0])
    t_obs = v_obs[f_obs].unsqueeze(dim=0)
    '''
    v_ind = igl.boundary_loop(f_obs)
    Points = v_obs[v_ind,:3]
    ...
    obs_distance = point_obstacle_distance(X, t_obs) - 1.1*offset
    #N_list = []
...
def my_sample_speed(input_file, numsamples, dim):
    v, f = igl.read_triangle_mesh(input_file)
    limit = 0.5
    margin = limit/12.0
    offset = margin/10.0 
    ...
    np.save("my_points.npy", sampled_points)
    np.save("my_speed.npy", speed)
...
if __name__ == '__main__':
    ...
    my_sample_speed(meshpath, sample_number, dim)
