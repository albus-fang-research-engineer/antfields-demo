...
class Model():
    ...
    def train(self):
        ...
        nominal_path_lengths = []      # xy arc length of nominal planned path
        ...
        while True:
            ...
            if self.mode != READ_FROM_COOKED_DATA:
                ...
                nominal_path_length = float(np.sum(np.linalg.norm(np.diff(nominal_np[:, :2], axis=0), axis=1)))
                ...
                nominal_path_lengths.append(nominal_path_length)
                ...
                if nominal_path_length > 0:
                    print(f"Added effort / path length: {added_effort / nominal_path_length:+.6f}")
                if call_seg_efforts:
    ...
    def predict_trajectory2_new(self, Xsrc, Xtar,
                            samples=200,
                            step_size=0.05,
                            tol=0.01,
    ...
    def policy_goal_direct(self, current_location, height):
        ...
        return traj_list, index - 1
    def reached_goal(self, current_pos, tol=0.007):
        """
