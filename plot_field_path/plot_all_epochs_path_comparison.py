...
# ── Figure layout ─────────────────────────────────────────────────────────────
n_epochs = len(EPOCHS)
fig = plt.figure(figsize=(5 * n_epochs + 7, 5))

# GridSpec only holds the 5 epoch panels + colorbar
gs = GridSpec(
    1, n_epochs + 1,
    width_ratios=[1] * n_epochs + [0.08],   # epochs | cbar
    wspace=0.15,
    left=0.04, right=0.66, bottom=0.12, top=0.78,
)

axes    = [fig.add_subplot(gs[0, i]) for i in range(n_epochs)]
cbar_ax = fig.add_subplot(gs[0, n_epochs])

# Comparison plot placed manually — far to the right of the colorbar
# [left, bottom, width, height] in figure-fraction coords
ax_cmp = fig.add_axes([0.78, 0.12, 0.13, 0.66])
...
# ── Comparison subplot: all planned paths overlaid (white bg, black walls) ────
...
for epoch, color in zip(EPOCHS, path_colors):
    traj_i = np.load(f'{BASE_PATH}/epoch_{epoch:04d}_optimized.npy')
...
ax_cmp.set_xlabel('X (m)', fontsize=16)
ax_cmp.set_title('Planned Path Across Epochs')

# Legend placed outside the comparison subplot, to its right
ax_cmp.legend(
    loc='center left',
...
leg2 = fig.legend(
    ...
    loc='upper center',
    bbox_to_anchor=(0.35, 0.736),
    ncol=1,
    borderaxespad=0,
    fontsize=16
)
fig.suptitle('Evolution of Planned Path and Neural Time Field',
             fontsize=16, fontweight='bold', x=0.35, y=0.9)
